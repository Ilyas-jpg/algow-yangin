...
export type FuelClass = "ORMAN" | "MAKI" | "OT" | "TARIM" | "YAPI" | "CIPLAK" | "SU";
...
function fuelOf(code: number | null): FuelClass | null {
 ...
 */
async function corineAt(lon: number, lat: number): Promise<FuelClass | null> {
  const d = 0.002;
  const url =
